...
// Description : Single AES-128 encryption round - purely combinatorial.
//               Implements SubBytes, ShiftRows, MixColumns, AddRoundKey.
//               is_final=1 skips MixColumns (round 10).
//
// TIMING NOTE: MixColumns uses only xtime() (×2) and xtime()^b (×3).
//              xtime = 1 shift + 1 conditional XOR ≈ 2 LUT levels.
//              ×3    = xtime(b) ^ b                ≈ 3 LUT levels.
//              This avoids deep combinatorial loops at 125 MHz.
// =============================================================================
...
module aes_round (
    ...
    //   = left-shift by 1, XOR 0x1B if bit 7 was set
    // -------------------------------------------------------------------------
    ...
    // State is treated as 4x4 array [row][col], column-major (FIPS-197)
    // Bit mapping: byte at (row, col) is at bits [127 - col*32 - row*8 -: 8]
    // -------------------------------------------------------------------------
    ...
    function automatic logic [127:0] set_byte (
        input logic [127:0] v,
        input int            row,
        input int            col,
        input logic [7:0]    val
    ...
    // -------------------------------------------------------------------------
    // Combinatorial round logic
    // -------------------------------------------------------------------------
    always_comb begin
        automatic logic [127:0] s, sb, sr, mc, rk, out;
        automatic logic [7:0]   t0, t1, t2, t3;
        ...
        for (int r = 0; r < 4; r++) begin
            for (int c = 0; c < 4; c++) begin
                sb = set_byte(sb, r, c, SBOX[get_byte(s, r, c)]);
            end
        end

        // --- 3. ShiftRows (row i shifted left by i) ---
        sr = 128'b0;
        // Row 0: no shift
        sr = set_byte(sr, 0, 0, get_byte(sb, 0, 0));
        sr = set_byte(sr, 0, 1, get_byte(sb, 0, 1));
        sr = set_byte(sr, 0, 2, get_byte(sb, 0, 2));
        sr = set_byte(sr, 0, 3, get_byte(sb, 0, 3));
        // Row 1: shift left 1
        sr = set_byte(sr, 1, 0, get_byte(sb, 1, 1));
        sr = set_byte(sr, 1, 1, get_byte(sb, 1, 2));
        sr = set_byte(sr, 1, 2, get_byte(sb, 1, 3));
        sr = set_byte(sr, 1, 3, get_byte(sb, 1, 0));
        // Row 2: shift left 2
        sr = set_byte(sr, 2, 0, get_byte(sb, 2, 2));
        sr = set_byte(sr, 2, 1, get_byte(sb, 2, 3));
        sr = set_byte(sr, 2, 2, get_byte(sb, 2, 0));
        sr = set_byte(sr, 2, 3, get_byte(sb, 2, 1));
        // Row 3: shift left 3
        sr = set_byte(sr, 3, 0, get_byte(sb, 3, 3));
        sr = set_byte(sr, 3, 1, get_byte(sb, 3, 0));
        sr = set_byte(sr, 3, 2, get_byte(sb, 3, 1));
        sr = set_byte(sr, 3, 3, get_byte(sb, 3, 2));

        // --- 4. MixColumns (skipped on final round) ---
        // FIPS-197 matrix: [2 3 1 1 / 1 2 3 1 / 1 1 2 3 / 3 1 1 2]
        // ×2 = xtime(b),  ×3 = xtime(b) ^ b
        mc = sr;  // default (final round passthrough)
        if (!is_final) begin
            for (int c = 0; c < 4; c++) begin
                t0 = get_byte(sr, 0, c);
                t1 = get_byte(sr, 1, c);
                t2 = get_byte(sr, 2, c);
                t3 = get_byte(sr, 3, c);
                mc = set_byte(mc, 0, c, xtime(t0) ^ (xtime(t1) ^ t1) ^ t2         ^ t3);
                mc = set_byte(mc, 1, c, t0         ^ xtime(t1)        ^ (xtime(t2) ^ t2) ^ t3);
                mc = set_byte(mc, 2, c, t0         ^ t1               ^ xtime(t2)        ^ (xtime(t3) ^ t3));
                mc = set_byte(mc, 3, c, (xtime(t0) ^ t0) ^ t1         ^ t2               ^ xtime(t3));
            end
        end
